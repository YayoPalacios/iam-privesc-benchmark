# `privesc-ssmSendCommand--user` — **confirmed false positive** (PMapper, region-scoped run)
...
```
user/privesc-ssmSendCommand-user can escalate privileges by accessing the administrative principal role/privesc-high-priv-service-role:
   user/privesc-ssmSendCommand-user can call ssm:SendCommand to access an EC2 instance with access to role/privesc-high-priv-service-role
```
...
`ssm:SendCommand` needs a live, SSM-registered target to act on. With zero managed nodes the
...
`ec2:RunInstances`; its grants are `ec2:DescribeInstances` plus the SSM SendCommand actions
...
- **cloudfox (admin-default):** **no FP.** Binds `ssm:SendCommand` to the principal in
  `permissions.csv` and reports `can ssm:SendCommand on *` in `iam-simulator`, both true
